...
本项目为 酷Q 的 CQHTTP 插件的 Python SDK 异步版本，采用异步 I/O，封装了 web server 相关的代码，支持 CQHTTP 的 HTTP 和反向 WebSocket 两种通信方式，让使用 Python 的开发者能方便地开发插件。同步版本见 [`python-cqhttp`](https://github.com/richardchien/python-cqhttp)。

关于 CQHTTP 插件，见 [richardchien/coolq-http-api](https://github.com/richardchien/coolq-http-api)；关于异步 I/O，见 [asyncio](https://docs.python.org/3/library/asyncio.html)。
...
async def handle_group_increase(event):
    await bot.send(event, message='欢迎新人～',
...
为了简化发送消息的操作，提供了 `send(event, message)` 函数，这里的第一个参数 `context` 也就是上报数据，传入之后函数会自己判断当前需要发送到哪里（哪个好友，或哪个群），无需手动再指定，其它参数仍然可以通过命名参数指定，例如 `auto_escape=True`。
...
后端运行了之后，需要配置 CQHTTP 插件。对于 HTTP 事件上报，需要配置 `post_url` 为 `http://host:port/`；对于反向 WebSocket 事件上报和 API 调用，分别需要配置 `ws_reverse_event_url` 和 `ws_reverse_api_url` 为 `ws://host:port/ws/event/` 和 `ws://host:port/ws/api/`。其中 `host` 和 `port` 均为 `bot.run()` 运行时的相应参数。
...
async def handle(event):
    ...
    await bot.send(event, Message('你好！') + MessageSegment.at(event.user_id))
    await bot.send(event, Message('你刚刚发了：') + event.message.extract_plain_text())
```

相关 API 文档见 [`MessageSegment`](https://nonebot.cqp.moe/api.html#class-messagesegment) 和 [`Message`](https://nonebot.cqp.moe/api.html#class-message)。
