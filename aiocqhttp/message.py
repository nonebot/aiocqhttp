...
class MessageSegment(dict):
    ...
    def contact_user(id_: int) -> "MessageSegment":
        """推荐好友。"""
        return MessageSegment(type_="contact", data={"type": "qq", "id": str(id_)})

    @staticmethod
    def contact_group(id_: int) -> "MessageSegment":
        """推荐群。"""
        return MessageSegment(type_="contact", data={"type": "group", "id": str(id_)})
    ...
    @staticmethod
    def music(type_: str, id_: int, style: Optional[int] = None) -> "MessageSegment":
        """音乐"""
    ...
    @staticmethod
    def node_custom(
        user_id: int, nickname: str, content: Message_T
    ) -> "MessageSegment":
        """合并转发自定义节点。"""
        ...
        for function_name, extra in iter_function_name_and_extra():
            if function_name == "text":
                if extra:
                    ...
                    yield MessageSegment(type_=function_name, data={"text": extra})
            else:
                data = {
                    k: v
                    for k, v in map(
